...
Inductive sized_usub : context -> expr -> expr -> expr -> nat -> Prop :=    (* defn sized_usub *)
 ...
 | ss_pi : forall (L:vars) (G:context) (A1 B1 A2 B2:expr) (k2 k1:kind) n1 n2 n3 n4 n5,
      ...
      G ⊢ A2 <: A1 : e_kind k1 | n3 ->
      (forall x, x \notin  L -> G , x : A1 ⊢ B1 <^> x <: B1 <^> x : e_kind k2 | n4)  ->
      (forall x, x \notin  L -> G , x : A2 ⊢ B1 <^> x <: B2 <^> x : e_kind k2 | n5)  ->
      G ⊢ e_pi A1 B1 <: e_pi A2 B2 : e_kind k2 | S (n1 + n2 + n3 + n4 + n5)
...
Ltac instantiate_LN_IH x :=
  repeat
    match goal with
    | H : forall (x : atom), x `notin` ?L -> exists n, _ |- _ =>
      let Fr := fresh "Fr" in
      let IH := fresh "IH" in
      assert (Fr : x `notin` L) by auto;
      specialize (H x Fr); clear Fr;
      rename H into IH
    | H : forall (x : atom), x `notin` ?L -> _ ⊢ _ <: _ : _ |- _ =>
       let Fr := fresh "Fr" in
       assert (Fr : x `notin` L) by auto;
       specialize (H x Fr); clear Fr
    end; destruct_exists
...
Ltac discover_fresh_dom x :=
  ...
  | H : ?Γ, x : ?A ⊢ _ <: _ : _ |- _ =>
    let Fr := fresh "Fr" in
    assert (Fr : x `notin` dom Γ) by
      (let H := fresh "H" in
       assert (H : ⊢ Γ, x : A) by auto; inversion H; now subst)
  end
.
...
Ltac instantiate_LN_IH_with_fresh :=
  let x := fresh "x" in
  pick fresh x; instantiate_LN_IH x;
...
Ltac find_IHtsz := match goal with
| H : forall _ _ _ _ _ _ _ _, forall_order _ <= _ -> _ |- _ => H
end.

Ltac find_IHesz := match goal with
| H : forall (_ : nat) _ _ _ _ _ _ _ _, forall_order _ <= _ -> _ |- _ => H
end.
...
Proof.
  ...
  [> now rewrite IHmono_type1, IHmono_type2
   | pick fresh x; specialize (H1 x Fr);
     rewrite open_var_forall_order_eq in H1;
...
Lemma sized_sub_wf : forall Γ e1 e2 A n,
    Γ ⊢ e1 <: e2 : A | n -> ⊢ Γ.
Proof.
  intros. apply sized_unsized in H. auto.
Qed.

Hint Resolve sized_weakening_cons : core.
Hint Resolve sized_sub_wf : core.

Ltac solve_right_forall_r_impl IH :=
  match goal with
  | _ : forall x, x `notin` ?L -> ?Γ, x : ?B ⊢ ?e2 <: ?C <^> x : * | ?n2
        |- ?Γ ⊢ ?e1 <: e_all ?B ?C : * => match goal with
    | _ : Γ ⊢ e1 <: e2 : * | ?n1 |- _ => match goal with
      | _ : Γ ⊢ B <: B : e_kind ?k | _ |- _ =>
        let x := fresh "x" in
        pick fresh x and apply s_forall_r;
        [> eauto 3.. |
         intros; apply IH with (n1 + n2) e2 * n1 n2; eauto 4;
         try rewrite open_var_size_eq;
         try rewrite open_var_forall_order_eq; simpl in *; omega]
      end
    end
  end
...
Ltac solve_right_forall_r :=
  let IH := find_IHesz in solve_right_forall_r_impl IH.

Ltac solve_left_forall_l_impl IH :=
  match goal with
  | _ : ?Γ ⊢ ?B0 <^^> ?e <: ?e2 : * | ?n1 |- ?Γ
        ⊢ e_all ?A ?B0 <: ?e3 : * => match goal with
    | _ : Γ ⊢ e2 <: e3 : ?B | ?n2 |- _ => match goal with
      | _ : Γ ⊢ A <: A : e_kind ?k | _ |- _ =>
        eapply s_forall_l; eauto;
        apply IH with (esize (B0 <^^> e) + esize e3) (n1 + n2) e2 B n1 n2; eauto;
        try (rewrite open_mono_order; auto); simpl in *; omega
      end
    end
  end
.
...
Ltac find_IHorder1 :=
  match goal with
  | H : forall (_ : nat) (_ : nat) _ _ _ _ _ _ _ _, forall_order _ <= _ -> _ |- _ => H
  end.

Ltac solve_left_forall_l :=
  let IH := find_IHorder1 in solve_left_forall_l_impl IH.
...
Ltac find_IHorder2 :=
  match goal with
  | H : forall (_ : nat) (_ : nat) (_ : nat) _ _ _ _ _ _ _ _,
      forall_order _ <= _ -> _ |- _ => H
  end
.

Ltac specialize_LN_hyps :=
  repeat match goal with
  | H : forall x, x `notin` ?L -> _ |- _ , ?x0 : _ ⊢ _ <: _ : _ =>
    let Fr := fresh "Fr" in
    assert (Fr : x0 `notin` L) by auto;
    specialize (H x0 Fr); clear Fr
  end
.


Ltac solve_with_IHorder2 :=
  specialize_LN_hyps; match goal with
  | Hl : ?Γ ⊢ ?e1 <: ?C <^^> ?e : * | ?n1
         |- ?Γ ⊢ ?e1 <: ?e3 : * => match goal with
    | Hr : Γ ⊢ C <^^> e <: ?e3 : * | ?n2 |- _ =>
      let IH := find_IHorder2 in
      apply IH with (forall_order e1 + forall_order e3)
                    (esize e1 + esize e3) (n1 + n2) (C <^^> e) * n1 n2; eauto;
      rewrite open_mono_order; auto; simpl in *; omega
     end
   end
.
...
Ltac solve_l_forall_r_r_forall_l :=
  prepare_left_judgement; solve_with_IHorder2.
...
Ltac solve_with_IHtsz :=
  ...
  | Hl : ?Γ ⊢ ?e1 <: ?e2 : ?A | ?n1 |- ?Γ ⊢ ?e1 <: ?e3 : _ =>
    ...
    | Hr : ?Γ ⊢ ?e2 <: ?e3 : ?B | ?n2 |- _ =>
      ...
      apply IH with e2 B n1 n2; eauto; simpl in *;
      try repeat (rewrite open_mono_order; auto);
      try repeat rewrite open_var_size_eq; omega
    end
  end
.
...
Ltac solve_with_IHesz :=
  match goal with
  | _ : ?Γ ⊢ ?e1 <: ?e2 : ?A | ?n1 |- ?Γ ⊢ ?e1 <: ?e3 : ?A =>
    match goal with
    | _ : ?Γ ⊢ ?e2 <: ?e3 : ?B | ?n2 |- _ =>
      let IH := find_IHesz in
      apply IH with (n1 + n2) e2 B n1 n2; eauto;
      repeat rewrite open_var_size_eq;
      repeat rewrite open_var_forall_order_eq;
      simpl in *; omega
    end
  end
.
...
Ltac solve_pi_return_type :=
  specialize_LN_hyps; unify_kinds;
...
Theorem transitivity' : forall order2 order1 esz tsz,
    forall Γ e1 e2 e3 A B n1 n2,
      forall_order e2 <= order2 ->
      forall_order e1 + forall_order e3 <= order1 ->
      esize e1 + esize e3 <= esz ->
      n1 + n2 <= tsz ->
      Γ ⊢ e1 <: e2 : A | n1 -> Γ ⊢ e2 <: e3 : B | n2 -> Γ ⊢ e1 <: e3 : A.
Proof.
  induction order2; induction order1; induction esz; induction tsz;
    intros * Eorder2 Eorder1 Eesz Etsz Subl Subr;
    extract_eqs.
  ...
  - solve_by_case_analysis.
  - solve_by_case_analysis.
  (* don't worry, the final case takes the longest time :) *)
  - solve_by_case_analysis.
Qed.
...
Proof.
  ...
  apply transitivity' with
      (forall_order e2) (forall_order e1 + forall_order e3)
