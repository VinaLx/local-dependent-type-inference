...
that arized in the design of the rules to ensure the desired properties.
...
to hold in L\"aufer and Odersky's system, these two rules rely on certain properties that
...
We define a ``extraction'' function that eliminates the type annotations
...
\begin{theorem}[Subsumption of DK's Type Well-formedness]
    If $\Gamma \vdash A$, then $\lift{\Gamma} \vdash \lift{A} : [[*]]$
\end{theorem}

\begin{theorem}[Subsumption of DK's Subtyping]
