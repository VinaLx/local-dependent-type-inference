...
then $[[G1 ,, [e1 / x] G2 |- [e1 / x] e : [e1 / x] B]]$) does not escape the issue above\bruno{Hummm is this text correctly
...
We define an erasure function\bruno{This is the function that is shown earlier, right?
  If so add a back reference to the figure here.} that eliminates the type annotations
...
\end{theorem}\bruno{You mean a rightarrow above instead of a left arrow, right?}
...
considering the subtyping aspect\bruno{Are you trying to show a simplified version of the lemma here?
...
can be demonstrated by the following derivation:\bruno{Do you mention this problem before to
