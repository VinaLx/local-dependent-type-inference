...
Worth mentioning is that while substitution of poly-expressions break the
...
We prove \emph{Generalized Transitivity} by induction on the 3-tuple:
...
not able to use the inductive hypothesis of $\mathcal{D}_1 + \mathcal{D}_2$, and
resort to the sizes of expressions ($\mathbf{size}(e_1) + \mathbf{size}(e_3)$)
...
We tackle this issue by employ designs to make the choice of implicit instantiation
...
We use another fact to solve the situation:
for a well-typed expression whose type can reduce, that expression
...
down into two aspects: a erasure-annotation process and the subtype preservation.
