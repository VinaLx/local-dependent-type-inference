...
\rref{r-inst,r-cast-inst}. The indeterminacy is caused by the guess of $e$,
...
\subsubsection{Extraction of Expression}

We tackle the issue of determinancy which sabotages the type-safety of our system
with a similar design to $\mathrm{ICC}^*$ \cite{barras2008implicit}, the extracted expression.
Figure \ref{fig:extraction} shows the syntax of the extracted expression and
the accompanied operational semantics. They essentially mirrors the syntax and semantics
of unextracted expression except the elimination of type annotations in $\lambda$,
$\Lambda$, $\mu$ and $\castup$ expressions. Restrictions are imposed in the
...
So that the implicit parameter can be directly eliminated in \rref{er-elim,er-cast-inst}.
For a well-typed expression, the reduction of its extraction is deterministic.
The type safety of our system is built around this idea and is discussed in \ref{sec:type-safety}.
