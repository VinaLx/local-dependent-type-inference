...
by not eliminating the implicit binder altogether, so that the correspondence
between the original and the erased expression can be established easier.
...
obtain ``polymorphic kinds'' like $[[forall x : int. *]]$ through \rref{s-forall-l,s-forall-r,s-sub},
...
where $[[B]]$ is required to only have kind $[[*]]$ but not $[[box]]$,
in contrast to \rref{s-pi} where $[[B]]$ at a similar position can be of any kind.
As a result, implicit polymorphic functions at type level are never well-typed.
...
Moreover, type-level function applications that involves implicit
abstractions ($\Lambda$ expressions) are also never well-typed
because of the restriction. So well-typed non-deterministic implicit instantiation at
type-level can never occur.
