...
And case analyses of terms are encoded to the application of the CPS functions.
...
Here the syntax includes types that are universally quantified over type parameters
($\forall$ types) as "polymorphic types" (or polytypes). The definition of monotypes
is standard, which includes all types without occurences of universal quantifiers.
...
The Hindley-Milner's declarative system also has a \textsc{GEN} rule to
